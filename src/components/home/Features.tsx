...
const Features = () => {
  ...
  return (
    <section className="py-8 sm:py-12 lg:py-16 bg-gradient-to-b from-slate-800 via-emerald-900/50 to-slate-800">
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {features.map(feature => (
              <Card key={feature.id} className={`card-hover glass-effect border border-emerald-400/30 h-full flex flex-col group transition-all duration-500 hover:border-emerald-400/60 hover:shadow-neon`}>
                <CardHeader className={`bg-gradient-to-r ${feature.gradient} text-center sm:text-left backdrop-blur-sm rounded-t-lg`}>
                  ...
                  <CardTitle className="text-lg sm:text-xl lg:text-2xl font-orbitron text-emerald-100">{feature.title}</CardTitle>
                </CardHeader>
                <CardContent className="flex-grow p-4 sm:p-6">
                  <CardDescription className="text-sm sm:text-base text-emerald-200 leading-relaxed">
                    {feature.description}
                  </CardDescription>
                </CardContent>
                <CardFooter className="p-4 sm:p-6 pt-0">
                  <Link 
                    to={feature.link} 
                    className="text-emerald-400 hover:text-emerald-300 font-medium transition-colors flex items-center text-sm sm:text-base font-orbitron group-hover:animate-pulse"
                  >
                    Learn More
